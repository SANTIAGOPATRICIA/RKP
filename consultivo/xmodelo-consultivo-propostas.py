...
with dados:
    st.markdown('**Informação para a proposta**')
    input_objeto = st.text_area(label="Insira o objeto da proposta: ")
    resumo_objeto = st.text_input(label="Resuma o objeto em uma frase: ")
    ...
    perguntas_respostas={'[objeto_texto]': input_objeto,
...
#formatando tipo e tamanho da fonte do documento
fonte_name_and_size(document, 'Arial', 12)

#Adicionar uma sessão ao documento
section = add_section(document,-1, 4, 2.5, 2, 3)
...
paragraph_format.alignment = 2  # direita
# paragraph_date = create_paragraph(document, f'Brasília/DF, {data_extenso(data)}', alignment=2, line_spacing=10)
...
format_paragraph(p_de, 3, 0, 124,8,0)

#paragrafo interessado
paragraph_para = document.add_paragraph()
paragraph_para.add_run('PARA: [cliente]  (Interessado)').bold = True
format_paragraph(paragraph_para, 3,0,0,48,18)

#paragrafo referencia
paragraph_ref = document.add_paragraph()
paragraph_ref.text = 'Referência: PROPOSTA DE HONORÁRIOS ADVOCATÍCIOS'
format_paragraph(paragraph_ref, 3, 0, 18,96,18)


#paragrafo primeiro
paragraph_one = document.add_paragraph()
format_paragraph(paragraph_one, 3, 1.5748, 18,18,18)
...
add_formatted_text(paragraph_one, full_text, bold_text)
...
# #paragrafo atuacao
# paragrah_atuacao = document.add_paragraph("A atuação desse Jurídico compreenderá as seguintes atividades:")
# format_paragraph(paragrah_atuacao, 3, 1.5748, 18,18,18)
...
#     paragraph_itens_atuacao.alignment = 0
...
# table.style = None #'LightShading-Accent1'
# # get table data -------------
# items = (
...
# block_three_two_valor_total.add_run(f'Valor total: R${valor_total} ({num_extenso(valor_total)})').bold = True
...
#     paragraph_three_three.add_run(f': Tendo em vista a parceria para com o cliente, a Roque Khouri & Pinheiro, por mera liberalidade e apenas no trabalho específico, concede o desconto de {desconto_percentual}% (xxxx por cento) em todos os valores descritos, totalizando assim, R${total_final} ({num_extenso(total_final)}) pela prestação de serviços contratados.')
...
# Salvar o documento
document.save("proposta_servicos_advocaticios.docx")
