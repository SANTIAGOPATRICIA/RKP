...
with dados:
    ...
    pronome_tratamento_representante = st.selectbox('tratamento respresentante', ('do senhor', 'da senhora'))
    nome_representante = st.text_input(label='Informe o nome do(a) representante')
    #input do objeto consultivo
    objeto_consultivo = st.text_area(label='Insira o objeto da proposta consultiva')
    resumo_consultivo = st.text_area(label='Forneça um resumo do objeto do consultivo')
    objeto_contencioso = st.text_area(label='Insira o objeto da proposta contencioso')

    #atividades da atuação juridica     
    atividades_atuacao = st.text_area(label='Informe as atividades da atuação. Dê enter para separá-los')
    ...
    perguntas_respostas = {
        ...
        '[objeto_consultivo]': objeto_consultivo,
        '[resumo_consultivo]': resumo_consultivo,
        '[objeto_contencioso]': objeto_contencioso,
        '[atividades_atuacao]': atividades_atuacao,
...
paragragh_representante = document.add_paragraph()
paragragh_representante.add_run(f'Aos cuidados {pronome_tratamento_representante} {nome_representante}').bold = True
paragraph_format = paragragh_representante.paragraph_format
paragraph_format.line_spacing = Pt(18)
...
# Objeto da proposta
paragrafo_objeto = document.add_paragraph(f'Conforme solicitação, apresentamos proposta de honorários para atuação consultiva, referente a {objeto_consultivo}, e atuação judicial contenciosa, no que tange aos {objeto_contencioso}.')
...
itens_para_mostrar = []
atuacao = perguntas_respostas['[atividades_atuacao]'].split("\n")
if len(atuacao) > 1:
    for atuar in itens_atuacao:
        itens_atuacao.append(atuar)

    else:   
        # Substituir [atividade_atuacao] pelo valor de resumo_objeto
        itens_atuacao = [item.replace("[atividades_atuacao]", itens_atuacao) for item in itens_atuacao]
        itens_para_mostrar.append(itens_atuacao)

for item in atividades_atuacao:
...
paragraph_five.text ='A Roque Khouri & Pinheiro Advogados Associados alerta que qualquer ação judicial implica em risco, estando a interessada ciente principalmente da possibilidade de condenação em honorários advocatícios, conforme previsto no Código de Processo Civil (10% a 20% sobre o valor da causa atualizado), principalmente diante das especificidades da fase atual em que o processo se encontra. Alerta também se tratar de análise e de confecção de peças a serem elaboradas com base no direito, jurisprudência atual e principalmente das informações e documentos que serão sempre fornecidos pela Interessada.'
...
table.style =  'LightShading-Accent1' #None
# get table data -------------
items = (
...
#paragrafo III-II
paragraph_three_two = document.add_paragraph('Os honorários advocatícios devidos em consequência da prestação de serviços previstas no item I seriam assim determinados:')
format_paragraph(paragraph_three_two, 3, 1.5748, 18,18,18)

#Bloco dos valores
block_three_two_resumo = document.add_paragraph(f'Atuação consultiva {resumo_consultivo}')
...
with desenvolvimento:
    ...
    st.write(f'**{paragragh_representante.text}**')
    ...
    while True:
        if objeto_consultivo:
            break
        time.sleep(2)
    st.write(paragrafo_objeto.text)
